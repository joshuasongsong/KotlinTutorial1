...
{
    private val recordButton: ImageButton by bind(R.id.record_button)
    private var isRecording: Boolean = false
    private val recordVolumn: ProgressBar by bind(R.id.record_volumn)

    private lateinit var fileName: String
    private var myRecorder: MyRecoder? = null
    ...
    {
        override fun doInBackground(vararg _args: Void): Void?
        {
            while (isRecording)
            {
                publishProgress()
                ...
                {
                    Log.d("RecordActivty", e.toString())
                }
            }
            return null
        }

        override fun onProgressUpdate(vararg _values: Void)
        {
            recordVolumn.progress = myRecorder?.amplitudeEMA?.toInt() ?: 0
        }
    }

    //執行錄音並且可以取得麥克風音量的錄音物件
    private inner class MyRecoder internal constructor(private val _output: String)
    {
        ...
        fun start()
        {
            if(recorder == null)
            {
                ...
                //在prepare函式之前呼叫
                recorder!!.setOutputFile(_output)
